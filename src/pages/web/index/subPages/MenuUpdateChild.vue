...
export default {
  ...
  props: ['data', 'index', 'curData'],
  methods: {
    init () {
      this.ruleForm.subName = this.curData.subName
      this.ruleForm.toUrl = this.curData.toUrl
    },
    returnPage () {
    ...
    submitForm (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          newArr.splice(this.index, 1, this.ruleForm)
          ...
          Http.postQfModel(`indexs/menu/update?id=${this.data._id}`, param, function (data) {
            ...
            if (data.nModified === 1) {
              ...
              parent.isShowUpdateChildForm = false
              parent.isShowTable = true
              parent.init()
            } else {
              Message({
