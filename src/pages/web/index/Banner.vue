<template>
  <div id="banner">
    ...
    <div class="btns">
      <el-button @click="changeAddState">添加banner</el-button>
...
export default {
  ...
  methods: {
    ...
    init () {
      let _this = this
      Http.getQfModel('indexs/banner', '', function (data) {
    ...
    sortConfirm () {
      ...
      indexs.forEach(function (e, i) {
        Http.postQfModel('indexs/banner/update?id=' + e.id, {bIndex: e.index}, function (data) {
          if (data === 0) {
            return
          }
          if (data.nModified === 1) {
    ...
    handleDelete (row) {
      ...
      MessageBox.confirm('此操作将删除该选项, 是否继续?', '提示', {
        ...
        Http.postQfModel('indexs/banner/delete', {id: row._id}, function (data) {
    ...
    submitForm (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          Http.postQfModel('indexs/banner/add', param, function (data) {
    ...
    submitForm1 (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          Http.postQfModel('indexs/banner/update?id=' + this.changeForm.id, param, function (data) {
            console.log(11111, data)
            if (data === 0) {
              Message({
                message: '修改失败！！！',
